# Todo

**Clean-up**

- [ ] Double clicking contents should not select the node as well (?)

**Packaging**

- [ ] Test on Linux
- [ ] Test on Windows
- [ ] Test on Mac

---

**Testing**

- [ ]

**Final**

- [ ] Change repo name
- [ ] Change About page

---

**Source import**

- [ ] Browser extension to import texts.

**Packages**

- [ ] BDRC???
- [ ] treasory of lives

**Clean-up**

- [ ] Shortcuts + display keyboard shortcuts on hover
- [ ] Wrapping nodes
- [ ] Rerun all crawls, on another machine.
- [ ] Transliterations assigned to entities to aid navigating the database
- [ ] Modify Wiki crawls to gather Romaji as well
- [ ] VIAF↔Wikidata concordance — **wired in Grognard**; still need pack build/publish (`npm run wikidata:viaf-concordance` after re-extract — see [authoritypacks README](../authoritypacks/README.md))

**Documentation**

- [ ] Website (shortcut guide)
- [ ] Figure out what to do about external documentation
